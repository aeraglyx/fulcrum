	...
	if context.region.alignment != 'RIGHT':
		...
		if bpy.data.is_saved:
			if is_current_file_version():
				...
				layout.operator("fulcrum.go_to_latest_version", text="Go to Latest", icon='LOOP_FORWARDS')
			layout.operator("fulcrum.save_as_new_version", text="Save as New", icon='DUPLICATE')
		else:
...
class FULCRUM_PT_node_tools(NodePanel, bpy.types.Panel):
	...
	def draw(self, context):
		...
		orange = row.operator("fulcrum.set_node_color", text="", icon='SEQUENCE_COLOR_02')
		orange.color = [0.40, 0.27, 0.19]
		yellow = row.operator("fulcrum.set_node_color", text="", icon='SEQUENCE_COLOR_03')
		yellow.color = [0.40, 0.40, 0.30]
		green = row.operator("fulcrum.set_node_color", text="", icon='SEQUENCE_COLOR_04')
		green.color = [0.24, 0.35, 0.26]
		blue = row.operator("fulcrum.set_node_color", text="", icon='SEQUENCE_COLOR_05')
		blue.color = [0.25, 0.34, 0.39]
		purple = row.operator("fulcrum.set_node_color", text="", icon='SEQUENCE_COLOR_06')
