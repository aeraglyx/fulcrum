...
class FULCRUM_OT_compare(bpy.types.Operator):
    ...
    bl_description = (
        "Compare speed of multiple nodes, useful for making optimized node groups"
    )
    ...
    def execute(self, context):
        ...
        def prepare_nodes():
            if (
                node.outputs[0].type == "SHADER"
            ):  # TODO fix when i select output (or any other nodes without out?)
        ...
        def pre_render():
            context.scene.render.resolution_percentage = 5
        ...
        def student(mean_a, mean_b, var_a, var_b, n):
            ...
            if mean_a == mean_b:
                ...
                t = (mean_a - mean_b) / math.sqrt(
                    (var_a + var_b) / n
                )  # TODO - stdev or variance, wtf?
            ...
            if var_a == 0 and var_b == 0:
                ...
                out = 1 - 1 / (
                    1 + math.e ** (0.000345 * z**5 - 0.069547 * z**3 - 1.604326 * z)
                )
        ...
        )  # TODO samples prob only work in cycles now
        ...
        for i, node in enumerate(selected):
            ...
            render_time = statistics.mean(
                data
            )  # TODO make it work for just 1 frame per node
        ...
        if self.use_base == True:
            smooth = 0.125
            props.result = falloff(max_time / base_time, smooth) / falloff(
                min_time / base_time, smooth
            )
        else:
        ...
        if self.frames > 1 and min_time != max_time:
            ...
            props.confidence = student(
                min_time, max_time, min_dev, max_dev, self.frames
            )  # takes variance
        else:
            props.confidence = 0
        ...
        for node in selected:
            ...
            if min_time == max_time:
                ...
                # s = lerp(props.confidence, 0.5, 1, 0.01, 0.1)
        ...
        self.report(
            {"INFO"},
            f"Done! Ratio: {props.result:.3f}, Confidence: {100*props.confidence:.0f}%",
        )
        return {"FINISHED"}
