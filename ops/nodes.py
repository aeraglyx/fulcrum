...
class FULCRUM_OT_align_nodes(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        while True:
            ...
            full_height = sum([node_height(node) for node in nodes]) + spacing_y * (
                len(nodes) - 1
            )
    ...
    def execute(self, context):
        ...
        for node in nodes:
            if node.type == "FRAME" or node.type == "REROUTE":
                continue
            node_center += node.location + node.dimensions * mathutils.Vector(
                (0.5, -0.5)
            )
            n += 1
...
class FULCRUM_OT_color_node_flow(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        node_ab = {
            node: mathutils.Vector((random.uniform(-1, 1), random.uniform(-1, 1)))
            for node in nodes
        }
        node_z = {node: node.location.x * 0.01 for node in nodes}
        xy_dist = {
            pair: (node_center(pair[1]) - node_center(pair[0])).length
